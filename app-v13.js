import { CONFIG } from './config-v13.js';
...
function homePage(){
  ...
  <h3 class="sectionLabel">Next milestone</h3><div class="milestone"><div><span class="eyebrow">HARDWARE V1</span><b>Bench electronics → 12 servos → structure → calibration</b></div><button class="btn soft" data-action="go-project">Open</button></div><h3 class="sectionLabel">System</h3><div class="statgrid"><div class="stat"><b class="ok">${data.robot.emergency?'STOP':'OK'}</b><span>Safety state</span></div><div class="stat"><b>${data.servos.filter(s=>s.enabled).length}/12</b><span>Servos enabled</span></div></div>`;
}

function projectsPage(){
  const tabItems=[['overview','Overview'],['build','Build'],['electronics','Electronics'],['software','Software'],['bom','Parts & BOM'],['tests','Tests'],['problems','Problems'],['decisions','Decisions'],['files','Files']];
  let content='';
  ...
  <div class="grid2">${tile('Build Guide','Sequência V1','tool','lav','show-build')}${tile('Electronics','Arquitetura & potência','tool','amber','show-electronics')}${tile('Software','Robot Core + protocol','projects','blue','show-software')}${tile('Notes','Project notes','file','','add-note')}</div>
  ...
  if(projectTab==='software') content=`<div class="panel"><h3>Robot Core V1</h3><p>A app já está preparada para Simulator / Real Robot. O firmware de base acompanha esta atualização como <code>H180_ROBOT_CORE_V1.ino</code>.</p></div>${['Communication','Servo Engine','Motion Engine','Sensor Engine','Balance Engine','Behaviour Engine','Safety Engine','Brain Interface'].map((x,i)=>row(x,i<4?'V1 core':'Prepared architecture','','<span class="badge green">'+(i<4?'CORE':'READY')+'</span>')).join('')}<div class="panel"><h3>Protocol</h3><p><code>GET /status</code> · <code>POST /command</code> · high-level commands first. Real servo output remains disabled in the starter firmware until explicitly enabled.</p></div>`;
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('./sw-v13.js').catch(console.warn));
