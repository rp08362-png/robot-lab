import { CONFIG } from './config-v110.js';
...
function nav(){const n=[['home','Home','home'],['projects','Projects','projects'],['ideas','Ideas','idea'],['library','Library','library'],['more','More','more']];const active=(page==='cad'||page==='csv')?(activeProjectId?'projects':'more'):page;return `<nav class="nav">${n.map(([k,l,i])=>`<button class="${active===k?'active':''}" data-nav="${k}"><b>${icon(i)}</b><span>${l}</span></button>`).join('')}</nav>`;}
...
function projectManualsHTML(pid){
  ...
  return `<div class="panel manualsPanel">
    ...
    <p>This area is reserved only for project manuals. Manuals do not appear in the generic Files section.</p>
  ...
  <div class="manualList">${arr.map(x=>`<div class="listitem manualItem">
    ...
    <button class="btn soft" data-openfile="${x.id}">Open</button>
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('./sw-v110.js').catch(console.warn));
