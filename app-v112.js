import { CONFIG } from './config-v112.js';
...
let lastAppNavigationSignature = null;

function appNavigationState(scrollY=0){
  return {
    robotLab:true,
    page,
    activeProjectId,
    ...
    moreTab,
    libraryTab,
    ideasTab:window.__ideasTab||'all',
    manualGroup,
    scrollY:Number(scrollY)||0
  };
}
function appNavigationSignature(){
  return JSON.stringify([
    page,activeProjectId,projectTab,controlTab,moreTab,libraryTab,
    window.__ideasTab||'all'
  ]);
}
function syncAppHistoryBeforeRender(){
  if(!session||restoringAppHistory)return;
  const sig=appNavigationSignature();
  if(lastAppNavigationSignature===null){
    history.replaceState(appNavigationState(window.scrollY||0),'',location.href);
    lastAppNavigationSignature=sig;
    return;
  }
  if(sig!==lastAppNavigationSignature){
    // Store the scroll position of the screen we are leaving.
    if(history.state?.robotLab){
      history.replaceState({...history.state,scrollY:window.scrollY||0},'',location.href);
    }
    history.pushState(appNavigationState(0),'',location.href);
    lastAppNavigationSignature=sig;
  }else if(history.state?.robotLab){
    history.replaceState({...history.state,...appNavigationState(history.state.scrollY||0)},'',location.href);
  }
}
function restoreAppNavigation(state){
  if(!state?.robotLab)return false;
  restoringAppHistory=true;
  page=state.page||'home';
  activeProjectId=state.activeProjectId||null;
  projectTab=state.projectTab||'overview';
  controlTab=state.controlTab||'manual';
  moreTab=state.moreTab||'settings';
  libraryTab=state.libraryTab||'all';
  window.__ideasTab=state.ideasTab||'all';
  manualGroup=state.manualGroup||'left-leg';
  lastAppNavigationSignature=appNavigationSignature();
  render();
...
function bundleCategory(path='',csvHeaders=[]){
  ...
  if(/(^|\/)manuals?(\/|$)/i.test(p)&&['pdf','doc','docx','odt','rtf','txt','md','html','htm'].includes(e))return 'manuals';
...
function bundleCategoryLabel(cat){
  return ({
    metadata:'Metadata',manuals:'Manuals','source-truth':'Source of Truth','3d-models':'3D Models','mechanical-cad':'Mechanical / CAD',
...
function nav(){const n=[['home','Home','home'],['projects','Projects','projects'],['ideas','Ideas','idea'],['library','Library','library'],['more','More','more']];const active=(page==='cad'||page==='csv'||page==='manual')?(activeProjectId?'projects':'more'):page;return `<nav class="nav">${n.map(([k,l,i])=>`<button class="${active===k?'active':''}" data-nav="${k}"><b>${icon(i)}</b><span>${l}</span></button>`).join('')}</nav>`;}
...
function projectWorkspacePage(project){
  ...
  const tabsGeneric=[['overview','Overview'],['manuals','Manuals'],['notes','Notes'],['ideas','Ideas'],['resources','Resources'],['files','Files']];
  let content='';
  if(projectTab==='overview') content=`<div class="panel projectOverview"><div class="split"><div><span class="eyebrow">${esc(project.type||'PROJECT')}</span><h3>${esc(project.name)}</h3></div>${badge(project.status||'Active')}</div><p>${esc(project.note||'Sem descrição.')}</p><p class="tiny">Criado ${fmtDate(project.createdAt||project.updatedAt||now())}</p></div>
    <div class="grid2">${tile('Ideas',`${projectIdeas(project.id).length} linked`,'idea','amber','project-ideas')}${tile('Resources',`${projectResources(project.id).length} linked`,'library','blue','project-resources')}${tile('3D / CAD','Open GLB viewer','tool','lav','open-cad')}${tile('Manuals',`${scoped('files',project.id).filter(x=>x.category==='manuals').length} docs`,'file','blue','project-manuals')}${tile('Notes',`${scoped('notes',project.id).length} notes`,'file','','project-notes')}${tile('Files',`${scoped('files',project.id).length} files`,'file','coral','project-files')}</div>`;
  if(projectTab==='manuals') content=projectManualsHTML(project.id);
...
function bundleProjectPage(project){
  const tabsBundle=[
    ...
    ['releases','Releases'],['manuals','Manuals'],['files','Files']
  ];
  let content='';
  ...
  if(projectTab==='overview'){
    content=`<div class="panel projectSourceHero"><div class="split"><div><span class="eyebrow">CURRENT PROJECT</span><h3>${esc(project.name)}</h3></div>${badge(project.currentRelease||project.version||'Release')}</div><p>${esc(project.note||'Project workspace.')}</p><div class="projectSourceMeta"><span>Source: ${esc(baseName(project.sourceTruthFile||'Source of Truth'))}</span><span>Updated: ${fmtDate(project.updatedAt||project.importedAt||now())}</span></div></div>
      <div class="bundleCategoryGrid">
        ...
        ${bundleCategoryCard('Manuals',scoped('files',pid).filter(x=>x.category==='manuals').length,'manuals','blue')}
      </div>
  ...
  if(projectTab==='manuals') content=projectManualsHTML(pid);
...
function humanoidProjectPage(project){
  const tabItems=[['overview','Overview'],['build','Build'],['electronics','Electronics'],['software','Software'],['bom','Parts & BOM'],['control','Control'],['manuals','Manuals'],['tests','Tests'],['problems','Problems'],['decisions','Decisions'],['ideas','Ideas'],['resources','Resources'],['files','Files']];
  let content='';
  if(projectTab==='overview') content=`<div class="panel"><div class="split"><div><span class="eyebrow">ROBOT PROJECT</span><h3>${esc(project.name)}</h3></div>${badge('V1')}</div><p>${esc(project.note||'')}</p><div class="progress"><i style="width:42%"></i></div><p class="tiny">V1 · eletrónica principal definida, 7/12 servos comprados, estrutura e calibração ainda por fechar fisicamente.</p></div>
    <div class="grid2">${tile('Control','Simulator / Robot','control','blue','project-control')}${tile('Calibration','12 DOF limits','cal','lav','project-calibration')}${tile('3D / CAD','Open GLB viewer','tool','coral','open-cad')}${tile('Manuals',`${scoped('files',project.id).filter(x=>x.category==='manuals').length} docs`,'file','blue','project-manuals')}${tile('Build Guide','Sequência V1','tool','','show-build')}${tile('Electronics','Arquitetura & potência','tool','amber','show-electronics')}</div>
...
function manualReaderPage(){
  const s=manualReaderState;
  const project=activeProjectId?projectLabel(activeProjectId):'Project';
  const ext=s.ext||extName(s.name);
  const isPdf=ext==='pdf';
  return `<button class="backBtn" data-action="manual-reader-back">← ${esc(project)}</button>
    ${sectionHead('MANUAL READER','Manuals',s.name||'Document reader')}
    <div class="panel manualReaderHeader">
      <div class="split"><div><span class="eyebrow">${esc(manualReaderLabel(s.name))}</span><h3>${esc(s.name||'Manual')}</h3></div>${badge(s.status==='ready'?'READY':s.status==='error'?'ERROR':'LOADING')}</div>
      <div class="manualReaderActions">
        ${s.url?`<button class="btn soft" data-action="manual-open-original">Open original</button>`:''}
        ${isPdf&&s.status==='ready'?`<button class="btn soft" data-action="manual-pdf-zoom-out">− Zoom</button><button class="btn soft" data-action="manual-pdf-zoom-in">+ Zoom</button>`:''}
      </div>
    </div>
    ${s.status==='loading'?'<div class="manualReaderLoading"><div class="spinner"></div><p>Reading document inside ROBOT LAB…</p></div>':''}
    ${s.status==='error'?`<div class="panel manualReaderError"><h3>Could not render this manual</h3><p>${esc(s.error||'Unsupported or invalid document.')}</p>${ext==='doc'?'<p class="tiny">Legacy binary .DOC is not safely parseable in this browser-only reader. Convert it to DOCX or PDF to read it inside ROBOT LAB.</p>':''}</div>`:''}
    ${s.status==='ready'&&isPdf?`<div class="pdfReaderToolbar"><button data-action="manual-pdf-prev" ${s.pdfPage<=1?'disabled':''}>←</button><span>Page <b id="manualPdfPage">${s.pdfPage}</b> / <b>${s.pdfPages}</b></span><button data-action="manual-pdf-next" ${s.pdfPage>=s.pdfPages?'disabled':''}>→</button></div><div class="pdfCanvasWrap"><canvas id="manualPdfCanvas"></canvas></div>`:''}
    ${s.status==='ready'&&!isPdf?`<article class="manualDocument">${s.html||`<pre>${escapeManualText(s.text||'')}</pre>`}</article>`:''}
    ${s.warnings?.length?`<div class="panel manualWarnings"><h3>Reader notes</h3>${s.warnings.map(w=>`<p class="tiny">${esc(w)}</p>`).join('')}</div>`:''}`;
}
async function openManualReader(fileId){
  const f=(data.files||[]).find(x=>x.id===fileId);if(!f)return;
  activeProjectId=f.projectId||activeProjectId;
  const ext=extName(f.name);
  manualReaderState={fileId,name:f.name,ext,url:'',status:'loading',error:'',html:'',text:'',pdfDoc:null,pdfPage:1,pdfPages:0,pdfScale:1.2,warnings:[]};
  page='manual';render();
  try{
    const url=await signedFileUrl(fileId);
    manualReaderState.url=url;

    if(ext==='pdf'){
      const pdfjs=await getPdfJs();
      const loading=pdfjs.getDocument({url});
      const pdf=await loading.promise;
      manualReaderState.pdfDoc=pdf;
      manualReaderState.pdfPages=pdf.numPages;
      manualReaderState.pdfPage=1;
      manualReaderState.status='ready';
      render();
    ...
    if(ext==='docx'){
      if(typeof mammoth==='undefined')throw new Error('DOCX reader engine is unavailable.');
      const arrayBuffer=await r.arrayBuffer();
      const result=await mammoth.convertToHtml({arrayBuffer},{
        externalFileAccess:false,
        includeDefaultStyleMap:true
      });
      manualReaderState.html=sanitizeManualHtml(result.value||'');
      manualReaderState.warnings=(result.messages||[]).map(m=>m.message||String(m)).slice(0,8);
    }else if(ext==='odt'){
      manualReaderState.html=sanitizeManualHtml(await odtToHtml(await r.arrayBuffer()));
    }else if(ext==='rtf'){
      manualReaderState.text=rtfToPlainText(await r.text());
      manualReaderState.html=`<pre>${escapeManualText(manualReaderState.text)}</pre>`;
    }else if(ext==='md'){
      manualReaderState.html=sanitizeManualHtml(simpleMarkdownToHtml(await r.text()));
    }else if(ext==='html'||ext==='htm'){
      manualReaderState.html=sanitizeManualHtml(await r.text());
    }else if(ext==='txt'){
      manualReaderState.text=await r.text();
      manualReaderState.html=`<pre>${escapeManualText(manualReaderState.text)}</pre>`;
    }else if(ext==='doc'){
      throw new Error('Legacy .DOC is not supported by the private in-app reader. Convert it to DOCX or PDF.');
    }else{
      throw new Error('This manual format is not supported by the in-app reader.');
    }

    manualReaderState.status='ready';
    render();
  }catch(e){
    console.error('Manual reader',e);
    manualReaderState.status='error';
    manualReaderState.error=e?.message||'Could not read this manual.';
    render();
  }
}
async function renderManualPdfPage(){
  const s=manualReaderState,canvas=$('#manualPdfCanvas');
  if(!canvas||!s.pdfDoc||s.status!=='ready')return;
  try{
    const pageObj=await s.pdfDoc.getPage(s.pdfPage);
    const base=pageObj.getViewport({scale:1});
    const wrap=canvas.parentElement;
    const available=Math.max(260,(wrap?.clientWidth||window.innerWidth)-24);
    const fitScale=available/base.width;
    const viewport=pageObj.getViewport({scale:Math.max(.35,fitScale*s.pdfScale)});
  ...
  }catch(e){console.error('PDF page render',e);toast('Could not render this PDF page.');}
}
function projectManualsHTML(pid){
  const arr=scoped('files',pid).filter(x=>x.category==='manuals');
  return `<div class="panel manualsPanel">
    <div class="split"><div><span class="eyebrow">PROJECT DOCUMENTS</span><h3>Manuals</h3></div>${badge(`${arr.length} files`)}</div>
    <p>This area is reserved only for project manuals. PDF, DOCX, ODT, RTF, TXT, Markdown and HTML can be read directly inside ROBOT LAB. Legacy DOC is kept as a file but should be converted to DOCX/PDF for in-app reading.</p>
    <label class="btn primary manualUploadBtn">+ Add Manual
      <input id="manualUpload" type="file" accept=".pdf,.doc,.docx,.odt,.rtf,.txt,.md,.html,.htm,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" hidden>
    </label>
  </div>
  <div class="manualList">${arr.map(x=>`<div class="listitem manualItem">
    <span class="manualDocIcon">${manualFileBadge(x.name)}</span>
    <div class="grow"><b>${esc(x.name)}</b><span>${manualFileType(x.name)} · ${cadFormatBytes(x.size||0)} · ${fmtDate(x.createdAt)}</span>${x.originalPath&&x.originalPath!==x.name?`<small>${esc(x.originalPath)}</small>`:''}</div>
    <button class="btn soft" data-openfile="${x.id}">${manualReaderSupported(x.name)?'Read':'Open'}</button>
    <button class="xbtn" data-delete="files:${x.id}">×</button>
  </div>`).join('')||'<div class="empty">No manuals yet. Add PDF, DOC/DOCX or another document manual.</div>'}</div>`;
}
function manualFileType(name=''){
  const e=extName(name).toUpperCase();
  return e==='PDF'?'PDF Manual':e==='DOCX'?'Word Manual':e==='DOC'?'Word Manual':e==='MD'?'Markdown Manual':e==='TXT'?'Text Manual':e||'Manual';
}
function manualReaderSupported(name=''){
  return ['pdf','docx','odt','rtf','txt','md','html','htm'].includes(extName(name));
}
function manualReaderLabel(name=''){
  const e=extName(name);
  return ({
    pdf:'PDF',docx:'Word / DOCX',odt:'OpenDocument',rtf:'RTF',
    txt:'Text',md:'Markdown',html:'HTML',htm:'HTML',doc:'Legacy Word'
  })[e]||e.toUpperCase()||'Document';
}
function escapeManualText(text=''){
  return String(text).replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
}
function simpleMarkdownToHtml(text=''){
  const src=String(text||'').replace(/\r\n?/g,'\n');
  const inline=s=>escapeManualText(s)
  ...
  for(const line of lines){
    if(/^```/.test(line)){
      closeLists();
      if(inCode){html+=`<pre><code>${escapeManualText(code.join('\n'))}</code></pre>`;code=[];inCode=False;}
      else inCode=True;
      continue;
    }
    ...
    if((m=line.match(/^(#{1,6})\s+(.+)$/))){
      closeLists();const n=m[1].length;html+=`<h${n}>${inline(m[2])}</h${n}>`;continue;
    }
    if((m=line.match(/^\s*[-*+]\s+(.+)$/))){
      if(inOl){html+='</ol>';inOl=False;}if(!inUl){html+='<ul>';inUl=True;}html+=`<li>${inline(m[1])}</li>`;continue;
    }
    if((m=line.match(/^\s*\d+\.\s+(.+)$/))){
      if(inUl){html+='</ul>';inUl=False;}if(!inOl){html+='<ol>';inOl=True;}html+=`<li>${inline(m[1])}</li>`;continue;
    }
    closeLists();
...
function sanitizeManualHtml(html=''){
  if(typeof DOMPurify==='undefined')return `<pre>${escapeManualText(String(html).replace(/<[^>]*>/g,' '))}</pre>`;
  return DOMPurify.sanitize(String(html),{
    USE_PROFILES:{html:true},
    ADD_ATTR:['target','rel'],
    ADD_DATA_URI_TAGS:['img'],
    FORBID_TAGS:['iframe','object','embed','script','style','form','input','button','video','audio'],
    FORBID_ATTR:['style','onerror','onclick','onload','srcset']
  });
}
function rtfToPlainText(rtf=''){
  ...
  s=s.replace(/\\par[d]?/gi,'\n')
     ...
     .replace(/\\u(-?\d+)\??/gi,(_,n)=>String.fromCharCode(Number(n)<0?Number(n)+65536:Number(n)))
     .replace(/\\[a-z]+-?\d* ?/gi,'')
     .replace(/[{}]/g,'')
     .replace(/\\\\/g,'\\');
  return s.replace(/\n{3,}/g,'\n\n').trim();
...
async function odtToHtml(arrayBuffer){
  if(typeof JSZip==='undefined')throw new Error('ODT reader needs the ZIP engine.');
  const zip=await JSZip.loadAsync(arrayBuffer);
  const entry=zip.file('content.xml');
  if(!entry)throw new Error('ODT content.xml is missing.');
  const xml=await entry.async('string');
  const doc=new DOMParser().parseFromString(xml,'application/xml');
  if(doc.querySelector('parsererror'))throw new Error('Could not parse ODT document.');
  const paras=[...doc.getElementsByTagNameNS('*','p'),...doc.getElementsByTagNameNS('*','h')];
  const seen=new Set(),blocks=[];
  for(const p of paras){
    if(seen.has(p))continue;seen.add(p);
    const text=(p.textContent||'').trim();
    if(!text)continue;
    const isHeading=(p.localName||'').toLowerCase()==='h';
    blocks.push(isHeading?`<h2>${escapeManualText(text)}</h2>`:`<p>${escapeManualText(text)}</p>`);
  }
  if(!blocks.length){
    const text=(doc.documentElement?.textContent||'').trim();
    return `<pre>${escapeManualText(text)}</pre>`;
  }
  return blocks.join('');
}
async function getPdfJs(){
  if(pdfJsModulePromise)return pdfJsModulePromise;
  pdfJsModulePromise=import('https://cdn.jsdelivr.net/npm/pdfjs-dist@5.7.284/build/pdf.min.mjs').then(pdfjs=>{
    pdfjs.GlobalWorkerOptions.workerSrc='https://cdn.jsdelivr.net/npm/pdfjs-dist@5.7.284/build/pdf.worker.min.mjs';
    return pdfjs;
  });
  return pdfJsModulePromise;
}
function resetManualReader(){
  manualReaderState={fileId:null,name:'',ext:'',url:'',status:'idle',error:'',html:'',text:'',pdfDoc:null,pdfPage:1,pdfPages:0,pdfScale:1.2,warnings:[]};
}

function manualFileBadge(name=''){
  const e=extName(name).toUpperCase();
  return ['PDF','DOC','DOCX','ODT','RTF','MD','TXT','HTML','HTM'].includes(e)?e.slice(0,4):'DOC';
}
function isManualDocument(file){
  const e=extName(file?.name||'');
  return ['pdf','doc','docx','odt','rtf','txt','md','html','htm'].includes(e);
}
async function uploadManual(file){
  if(!file)return false;
  if(!isManualDocument(file))return toast('Manuals accepts document files only: PDF, DOC/DOCX, ODT, RTF, TXT, MD or HTML.');
  return uploadFile(file,activeProjectId,true,'manuals');
}

function projectFilesHTML(pid){const arr=scoped('files',pid).filter(x=>x.category!=='manuals');return `<div class="panel"><h3>Project files</h3><p>Private files for this project. 3D, CAD and image files show a visual preview when the format can be rendered on-device.</p><label class="btn soft">+ Upload<input id="fileUpload" type="file" hidden></label></div>${arr.map(x=>{const kind=previewKind(x.name,x.type);return `<div class="listitem">${kind!=='file'?filePreviewSquare(x.id,x.name,x.type):`<span class="iconbubble">${icon(isGlbName(x.name)?'tool':'file')}</span>`}<div class="grow"><b>${esc(x.name)}</b><span>${x.category?esc(bundleCategoryLabel(x.category))+' · ':''}${esc(x.type||'file')} · ${fmtDate(x.createdAt)}</span>${x.originalPath&&x.originalPath!==x.name?`<small>${esc(x.originalPath)}</small>`:''}</div><button class="btn" data-openfile="${x.id}">${extName(x.name)==='csv'?'Read CSV':['glb','gltf','stl','step','stp'].includes(extName(x.name))?'Open 3D':'Open'}</button><button class="xbtn" data-delete="files:${x.id}">×</button></div>`}).join('')||'<div class="empty">No files.</div>'}`;}
...
function render(){
  ...
  syncAppHistoryBeforeRender();
  const title={home:'Home',projects:activeProjectId?projectLabel(activeProjectId):'Projects',ideas:'Ideas',control:'Control',library:'Library',cad:'3D / CAD Viewer',csv:'CSV Viewer',manual:'Manual Reader',more:'More'}[page];
  const body={home:homePage,projects:projectsPage,ideas:ideasPage,control:controlPage,library:libraryPage,cad:cadViewerPage,csv:csvViewerPage,manual:manualReaderPage,more:morePage}[page]();
  $('#app').innerHTML=shell(body,title);bind();drawHeaderState();hydrateVisualPreviews();if(page==='cad')bindCadViewer();if(page==='manual'&&manualReaderState.status==='ready'&&manualReaderState.ext==='pdf')requestAnimationFrame(renderManualPdfPage);
}
...
function bind(){
  ...
  $('#manualUpload')?.addEventListener('change',e=>{if(e.target.files?.[0])uploadManual(e.target.files[0]);});
...
function handleAction(a,e){
  ...
  else if(a==='manual-reader-back'){page='projects';projectTab='manuals';render();}
  else if(a==='manual-open-original'){if(manualReaderState.url)window.open(manualReaderState.url,'_blank','noopener');}
  else if(a==='manual-pdf-prev'){if(manualReaderState.pdfPage>1){manualReaderState.pdfPage--;render();}}
  else if(a==='manual-pdf-next'){if(manualReaderState.pdfPage<manualReaderState.pdfPages){manualReaderState.pdfPage++;render();}}
  else if(a==='manual-pdf-zoom-in'){manualReaderState.pdfScale=Math.min(2.5,manualReaderState.pdfScale+.15);render();}
  else if(a==='manual-pdf-zoom-out'){manualReaderState.pdfScale=Math.max(.55,manualReaderState.pdfScale-.15);render();}
  ...
  else if(a==='project-manuals'){projectTab='manuals';render();}
...
async function uploadFile(file,projectId=activeProjectId||null,rerender=true,category=null){if(!session)return false;toast(category==='manuals'?'Saving manual…':'A enviar ficheiro…');try{const path=`${session.user.id}/robot-lab/${category==='manuals'?'manuals/':''}${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,'_')}`;const r=await api(`${CONFIG.supabaseUrl}/storage/v1/object/${CONFIG.storageBucket}/${path}`,{method:'POST',headers:{'Content-Type':file.type||'application/octet-stream','x-upsert':'false'},body:file});if(!r.ok)throw new Error(await r.text());data.files.unshift({id:id(),name:file.name,type:file.type,size:file.size,path,projectId:projectId||null,category:category||null,createdAt:now()});markDirty();if(rerender)render();toast(category==='manuals'?'Manual saved.':'Ficheiro guardado.');return true;}catch(e){console.warn(e);toast('Não foi possível enviar. Mantive a aplicação sem alterações.');return false;}}
async function openStoredFile(fileId){
  const f=data.files.find(x=>x.id===fileId);if(!f)return;
  try{
    const e=extName(f.name);
    if(f.category==='manuals'){
      if(manualReaderSupported(f.name)){await openManualReader(fileId);return;}
      if(e==='doc'){activeProjectId=f.projectId||activeProjectId;manualReaderState={fileId,name:f.name,ext:e,url:await signedFileUrl(fileId),status:'error',error:'Legacy .DOC is not supported by the private in-app reader. Convert it to DOCX or PDF.',html:'',text:'',pdfDoc:null,pdfPage:1,pdfPages:0,pdfScale:1.2,warnings:[]};page='manual';render();return;}
    }
...
window.addEventListener('popstate',e=>{
  if(session&&e.state?.robotLab){
    restoreAppNavigation(e.state);
  }
});
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('./sw-v112.js').catch(console.warn));
