import { CONFIG } from './config-v18.js';
...
function bundleAssetListHTML(projectId,category,empty='No files in this section.'){
  ...
  return arr.map(a=>{
    ...
    return `<div class="listitem bundleAsset">${visual?filePreviewSquare(a.fileId,a.name,f?.type||a.type):`<span class="iconbubble">${icon(bundleAssetIcon(category))}</span>`}<div class="grow"><b>${esc(a.name)}</b><span>${esc(a.originalPath||a.name)}</span><small>${cadFormatBytes(a.size||0)}${a.release?` · ${esc(a.release)}`:''}</small></div><button class="btn" data-openfile="${a.fileId}">${category==='3d-models'?'Open 3D':'Open'}</button></div>`;
...
function bundleProjectPage(project){
  ...
  if(projectTab==='bom'){
    ...
    content=`<div class="panel"><h3>Bill of Materials</h3><p>Structured BOM data and original source files.</p></div>
...
async function importProjectBundle(file){
  ...
  try{
    ...
    project.note=meta.exactTP7?'TP-7 RADIO V1 · Source of Truth V0.13.':'Project workspace created from its Source of Truth.';
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('./sw-v18.js').catch(console.warn));
