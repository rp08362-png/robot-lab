import { CONFIG } from './config-v16.js';
...
function bundleProjectPage(project){
  ...
  return `<button class="backBtn" data-action="back-projects">← All Projects</button>${sectionHead('PROJECT BUNDLE',project.name,`${project.currentRelease||''} · Private imported workspace`)}${tabs(tabsBundle,projectTab,'project')}${content}`;
}
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('./sw-v16.js').catch(console.warn));
