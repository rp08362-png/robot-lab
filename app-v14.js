import { CONFIG } from './config-v14.js';
...
function nav(){const n=[['home','Home','home'],['projects','Projects','projects'],['ideas','Ideas','idea'],['library','Library','library'],['more','More','more']];return `<nav class="nav">${n.map(([k,l,i])=>`<button class="${page===k?'active':''}" data-nav="${k}"><b>${icon(i)}</b><span>${l}</span></button>`).join('')}</nav>`;}
...
function projectWorkspacePage(project){
  ...
  if(projectTab==='overview') content=`<div class="panel projectOverview"><div class="split"><div><span class="eyebrow">${esc(project.type||'PROJECT')}</span><h3>${esc(project.name)}</h3></div>${badge(project.status||'Active')}</div><p>${esc(project.note||'Sem descrição.')}</p><p class="tiny">Criado ${fmtDate(project.createdAt||project.updatedAt||now())}</p></div>
    <div class="grid2">${tile('Ideas',`${projectIdeas(project.id).length} linked`,'idea','amber','project-ideas')}${tile('Resources',`${projectResources(project.id).length} linked`,'library','blue','project-resources')}${tile('Notes',`${scoped('notes',project.id).length} notes`,'file','','project-notes')}${tile('Files',`${scoped('files',project.id).length} files`,'file','coral','project-files')}</div>`;
...
function humanoidProjectPage(project){
  ...
  if(projectTab==='overview') content=`<div class="panel"><div class="split"><div><span class="eyebrow">ROBOT PROJECT</span><h3>${esc(project.name)}</h3></div>${badge('V1')}</div><p>${esc(project.note||'')}</p><div class="progress"><i style="width:42%"></i></div><p class="tiny">V1 · eletrónica principal definida, 7/12 servos comprados, estrutura e calibração ainda por fechar fisicamente.</p></div>
    <div class="grid2">${tile('Control','Simulator / Robot','control','blue','project-control')}${tile('Calibration','12 DOF limits','cal','lav','project-calibration')}${tile('Build Guide','Sequência V1','tool','','show-build')}${tile('Electronics','Arquitetura & potência','tool','amber','show-electronics')}</div>
...
function projectFilesHTML(pid){const arr=scoped('files',pid);return `<div class="panel"><h3>Project files</h3><p>Ficheiros privados deste projeto.</p><label class="btn soft">+ Upload<input id="fileUpload" type="file" hidden></label></div>${arr.map(x=>`<div class="listitem"><span class="iconbubble">${icon('file')}</span><div class="grow"><b>${esc(x.name)}</b><span>${esc(x.type||'file')} · ${fmtDate(x.createdAt)}</span></div><button class="btn" data-openfile="${x.id}">Abrir</button><button class="xbtn" data-delete="files:${x.id}">×</button></div>`).join('')||'<div class="empty">Sem ficheiros.</div>'}`;}
...
function libraryPage(){
  const items=[['all','All'],['video','Videos'],['tutorial','Tutorials'],['docs','Docs'],['article','Articles'],['product','Products'],['global','Unassigned']];
...
function resourceHTML(r){const thumb=r.thumbnail||'';const links=(r.projectIds||[]).map(projectLabel).join(', ');return `<div class="listitem">${thumb?`<img class="thumb" src="${esc(thumb)}" alt="">`:`<span class="iconbubble">${icon(r.type==='video'?'video':'library')}</span>`}<div class="grow"><b>${esc(r.title)}</b><span>${esc(r.type)} · ${esc(r.tags?.join(', ')||'')}</span><small>${links?`Projects: ${esc(links)}`:'Global / Unassigned'}${r.url?` · ${esc(r.url)}`:''}</small></div><button class="btn" data-openurl="${esc(r.url)}">Abrir</button><button class="xbtn" data-delete="resources:${r.id}">×</button></div>`;}

function morePage(){
  const t=[['settings','Settings'],['backup','Backup']];let content='';
  if(moreTab==='settings') content=settingsHTML();
  if(moreTab==='backup') content=backupHTML();
  return `${sectionHead('ROBOT LAB','More','Segurança, ligação ao robot, preferências e backup.')}${tabs(t,moreTab,'more')}${content}`;
}
...
function render(){
  ...
  const title={home:'Home',projects:activeProjectId?projectLabel(activeProjectId):'Projects',ideas:'Ideas',control:'Control',library:'Library',more:'More'}[page];
  const body={home:homePage,projects:projectsPage,ideas:ideasPage,control:controlPage,library:libraryPage,more:morePage}[page]();
...
function handleAction(a,e){
  ...
  if(a==='all-projects'){activeProjectId=null;projectTab='overview';go('projects');}
    ...
    {name:'type',label:'Type',type:'select',options:['video','tutorial','docs','article','product']},
    {name:'projectId',label:'Associate with project',type:'select',options:projectOptions(true),value:''},
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('./sw-v14.js').catch(console.warn));
